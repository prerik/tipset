	...
	<div <?php post_class('post'); ?> id="post-<?php the_ID(); ?>"><!-- post div -->
		<h2 class="title<?php if(is_sticky()) {echo " sticky-h2";} ?>"><a href="<?php the_permalink(); ?>" title="<?php printf( __('Permalink to %s', 'zbench'), the_title_attribute('echo=0') ); ?>"><?php the_title(); ?></a></h2>
			...
			<span class="post-info-date">
				<?php _e('Posted by', 'zbench'); ?> <a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>" title="<?php printf( __( 'View all posts by %s', 'zbench' ), get_the_author() ) ?>" rel="author"><?php echo get_the_author_meta('first_name') ?> <?php echo get_the_author_meta('last_name') ?>(<?php the_author(); ?>)</a>
		...
		<div class="clear"></div>
                <?php echo get_avatar($post->post_author,$size='50',$default='' ); ?>
		<div class="entry">
